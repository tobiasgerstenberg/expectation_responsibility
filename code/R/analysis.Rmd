...
for(i in 1:length(demographics.list)){
  df.tmp = demographics.list[[names(demographics.list)[i]]] %>%
    ...
    mutate_each(funs(round(.,2)),-experiment)
...
agent.bad = function(beta,df,reward){
  df = df %>%
    mutate(
      outcome_counterfactual = ifelse(outcome == 1 & outcome_counterfactual == 1, probability_counterfactual, outcome_counterfactual),
      outcome_counterfactual = ifelse(outcome == 0 & outcome_counterfactual == 0, probability_counterfactual, outcome_counterfactual)) %>%
...
for(i in 1:length(situation.names)){
  ...
  exp_r_prior_scaled[[situation.names[i]]] = rescale(exp_r_prior[[situation.names[i]]],to=c(0,1),from = range(exp_r_prior[[situation.names[i]]]))
}
...
df.regression = rbind(df.regression.goalie,
  ...
  select(experiment,situation,outcome,difference,pivotality,rating,probability)

# list for regression fits
fit.regression = list()

# difference and pivotality
fit.regression[["difference_pivotality"]] = lm(rating~difference+pivotality,data=df.regression)
df.regression$p.difference_pivotality = fit.regression[["difference_pivotality"]]$fitted.values

# difference only
fit.regression[["difference"]] = lm(rating~difference,data=df.regression)
df.regression$p.difference = fit.regression[["difference"]]$fitted.values

# pivotality only
fit.regression[["pivotality"]] = lm(rating~pivotality,data=df.regression)
...
fit.regression[["optimality"]] = lm(rating~optimal*outcome,data=df.regression)
...
for(i in 1:length(model.names)){
  ...
  model.metrics$r[i] = cor(abs(df.regression$rating),abs(df.regression[[paste0("p.",model.names[i])]]))
  model.metrics$rmse[i] = rmse(abs(df.regression$rating),abs(df.regression[[paste0("p.",model.names[i])]]))
}
...
plot.responsibility.no_model = function(x){
  ggplot(x,aes(x=probability,y=rating,fill=outcome))+
    ...
    theme(text=element_text(size=20),
          ...
          plot.title = element_text(face = "bold", lineheight = unit(2,"cm"))
    )
}

# responsibility judgments with model predictions
plot.responsibility.with_model = function(x){
...
plot.inferences = function(x, experiment.name){
  ...
  ggplot(x,aes(x=probability,y=rating,group=agent,fill=agent))+
    stat_summary(fun.y = mean, geom = "bar",color="black",position=position_fill(),width=0.75) +
    facet_wrap(~outcome,ncol=1)+
    labs(y = "agent inference", fill = "type of agent:  ", x = "action")+
    scale_fill_manual(values = c("peru", rgb(0.7,0.7,0.7), "gold"),labels = agentlabels)+
...
- You can change the experiment.name and cluster.index variables to determine which plot is to be printed. 
...
df.plot = df.plot %>%
  mutate(probability = paste0(probability,'%'),
         outcome = factor(outcome,levels = c("wrong", "correct"), labels = c("negative outcome",
                                                                             "positive outcome"))) %>%
  merge(.,cluster.list[[i]]) %>%
  filter(cluster == j)
  
  p = plot.responsibility.no_model(df.plot) + ggtitle(paste0("Experiment: ", i, ", Cluster: ", j))
  print(p)
  }
}
...
# plot results
plot.responsibility.with_model(df.plot) + ggtitle(paste0("Experiment: ", experiment.name, ", Model: ", model.name))
...
df.plot = df.regression %>%
  mutate_each(funs(abs),p.difference,p.pivotality,p.difference_pivotality,p.optimality,rating) %>%
...
ggplot(df.plot,aes_string(x=paste0("p.",model.name),y="rating",color="experiment"))+
